...
const mix = (hex: string, alphaPct: number) =>
  `color-mix(in srgb, ${hex} ${alphaPct}%, transparent)`;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ borderColor: mix(c.ink, 10) }}
      >
        ...
        <div
          ...
          style={{ background: c.surface, color: mix(c.ink, 62) }}
        >
          {project.domain}
        </div>
...
function MockBody({ project }: { project: Project }) {
  ...
  const bar = (w: string, h = "0.55em", color?: string) => (
    <span
      ...
      style={{ width: w, height: h, background: color ?? mix(c.ink, 18) }}
    />
  );
  ...
  if (project.layout === "local") {
    const tiles = project.mockTiles ?? ["Signature offer", "Opening hours", "Find us"];
    return (
      <>
        <div className="mt-[2.4em] grid grid-cols-[1fr_1.05fr] items-center gap-[2.8em]">
          <div className="flex flex-col gap-[1.2em]">
            ...
            <div
              ...
              style={{ background: c.paper, border: `1px solid ${mix(c.ink, 12)}` }}
            >
          ...
          <div className="flex flex-col gap-[1em]">
            ...
            <div className="mt-[0.4em] flex items-center gap-[1em]">
              <span
                className="flex h-[2.5em] items-center rounded-full px-[1.9em] text-[1em] font-bold"
                style={{ background: c.accent, color: c.paper }}
              >
                {project.mockCta ?? "Book now"}
              </span>
              <span
                className="flex h-[2.5em] items-center rounded-full px-[1.9em] text-[1em] font-bold"
                style={{ border: `1px solid ${mix(c.ink, 25)}` }}
              >
                View menu
              </span>
            </div>
        ...
        <div className="mt-[2.2em] grid grid-cols-3 gap-[1.2em]">
          {tiles.map((t, i) => (
            <span
              key={t}
              className="flex h-[5.8em] flex-col justify-between rounded-[1em] p-[1.1em]"
              style={{ background: i === 1 ? c.surface : c.wash }}
            >
  ...
  if (project.layout === "editorial") {
    return (
      <>
              ...
              <div key={col} className="flex flex-col gap-[0.9em]">
                {[94, 100, 88, 62].map((w, i) => (
                  <span
                    ...
                    style={{ width: `${w}%`, background: mix(c.ink, 15) }}
                  />
                ))}
              </div>
            ))}
        ...
        <div
          className="mt-[2.4em] flex items-center justify-between rounded-[1.1em] px-[1.7em] py-[1.3em]"
          style={{ background: c.surface }}
        >
          <div className="flex items-center gap-[1.1em]">
            <span
              className="flex h-[2.4em] w-[2.4em] items-center justify-center rounded-full text-[1.1em] font-extrabold"
              style={{ background: c.paper, color: c.accent }}
            >
              {project.name.charAt(0)}
            </span>
          ...
          <span
            className="flex h-[2.3em] items-center rounded-full px-[1.6em] text-[1em] font-bold"
            style={{ background: c.accent, color: c.paper }}
          >
            {project.mockCta ?? "Book a visit"}
          </span>
        </div>
      </>
    );
    ...
    return (
      <>
        <div className="mt-[2.6em] flex flex-col items-center gap-[1.2em] text-center">
          ...
          <div className="mt-[0.4em] flex items-center gap-[1em]">
            <span
              ...
              style={{ background: c.accent, color: c.paper }}
            >
              {project.mockCta ?? "Start free trial"}
            </span>
            <span
              className="flex h-[2.5em] items-center rounded-full px-[1.9em] text-[1em] font-bold"
              style={{ border: `1px solid ${mix(c.ink, 25)}` }}
        ...
        <div
          ...
          style={{ borderColor: mix(c.ink, 12), background: c.surface }}
        >
          <div
            className="flex h-[2.6em] items-center gap-[1.4em] border-b px-[1.5em]"
            style={{ borderColor: mix(c.ink, 12) }}
          >
            {bar("3.4em", "0.45em")}
            {bar("3.4em", "0.45em")}
          ...
          <div className="flex gap-[1.6em] p-[1.5em]">
            <div className="flex w-[16%] flex-col gap-[1em]">
              {[0, 1, 2, 3].map((i) => (
                <span
                  key={i}
                  className="h-[1.4em] rounded-[0.5em]"
                  style={{ background: mix(c.ink, 10) }}
                />
              ))}
            </div>
          ...
          <div className="grid grid-cols-3 gap-[1.2em] p-[1.5em] pt-0">
            {[0, 1, 2].map((i) => (
              <span
                key={i}
                className="flex h-[4.6em] flex-col justify-between rounded-[0.9em] p-[1em]"
  ...
  // commerce / default — the classic product-storefront pattern
  return (
    <>
      <div className="mt-[2.6em] grid grid-cols-[1.15fr_1fr] items-center gap-[3em]">
        ...
        <div className="flex flex-col gap-[1.2em]">
          <div className="flex gap-[1.2em]">
            ...
            <span
              className="aspect-[1/1] flex-1 rounded-[1.1em]"
              style={{ background: mix(c.surface, 55) }}
            >
              <span className="block h-full w-full rounded-[1.1em]" style={{ background: mix(c.accent, 45) }} />
            </span>
          </div>
